...
def load_gazebo_models(table_pose=Pose(position=Point(x=0.75, y=0.0, z=0.0)),
                       ...
                       block_pose2=Pose(position=Point(x=0.5225, y=0.2265, z=0.7725))):
    # Get Models' Path
    model_path = rospkg.RosPack().get_path('picknplace')+"/models/"
    # Load Table SDF
    table_xml = ''
...
def main():
    ...
    pnp2 = PickAndPlace(limb, hover_distance)
    ...
    block_poses.append(Pose(
        position=Point(x=0.47, y=0.175, z=-0.169),
    ...
    block_poses.append(Pose(
        position=Point(x=0.57, y=0.215, z=-0.169),
        orientation=overhead_orientation))
    
    block_poses.append(Pose(
        position=Point(x=0.6, y=-0.1, z=-0.110),
        orientation=overhead_orientation))
    # Move to the desired starting angles
    print("Running. Ctrl-c to quit")
    pnp.move_to_start(starting_joint_angles)
    pnp2.move_to_start(starting_joint_angles)

    pnp.pick(block_poses[0])
    pnp.place(block_poses[1])
    pnp2.pick(block_poses[2])
    pnp2.place(block_poses[3])
